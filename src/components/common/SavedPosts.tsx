...
const SavedPostsComponent = function SavedPosts({ onCollapse, refreshKey, isAuthenticated, onLoginClick, initialExpanded, inline, highlighted }: SavedPostsProps) {
  ...
  <div data-save-target className={`lg:hidden fixed bottom-12 md:bottom-0 left-0 right-0 bg-white shadow-lg transition-transform duration-300 z-40 ${isCollapsed ? 'translate-y-[calc(100%-3rem)]' : 'translate-y-0'}`} style={{ maxHeight: '60vh', paddingBottom: 'env(safe-area-inset-bottom)' }}>
