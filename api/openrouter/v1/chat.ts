...
function transformResponseToAnthropic(openRouterResponse: Record<string, unknown>): Record<string, unknown> {
  const choices = openRouterResponse.choices as Array<{ message: { content: string; role: string }; finish_reason?: string }>;
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (!parseResult.success) {
      ...
      return createCorsResponse({
        error: errMsg,
        code: errMsg.includes('too large') ? 'PAYLOAD_TOO_LARGE' : 'INVALID_JSON'
